...
class DirectAccessTest extends StorageApiTestCase
{
    ...
    public function testDirectAccessAndRestrictions()
    {
        ...
        }, $connection->fetchAll(sprintf('SHOW COLUMNS IN %s', $connection->quoteIdentifier('mytable_displayName'))));
        ...
        $columns = array_map(function ($row) {
            return $row['column_name'];
        }, $client2Connection->fetchAll(sprintf('SHOW COLUMNS IN %s', $connection->quoteIdentifier($tableName))));
