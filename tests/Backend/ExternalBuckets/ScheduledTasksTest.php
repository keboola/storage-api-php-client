...
class ScheduledTasksTest extends StorageApiTestCase
{
    ...
    public function testSchedulerWorker(): void
    {
        ...
        sleep(63); // Wait for the Scheduler restart (every 60 seconds)
        ...
        sleep(63); // Wait for the Scheduler restart

        // Check: Table created in workspace is not available in bucket because of stopped auto-refresh
        $tablesOutdated = $this->_client->listTables($bucketId);
        $this->assertCount(1, $tablesOutdated);

        $this->_client->refreshBucket($bucketId);

        // Check: Table created in workspace is after manual refresh available in bucket
