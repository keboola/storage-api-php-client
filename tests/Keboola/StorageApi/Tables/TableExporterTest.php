...
class Keboola_StorageApi_Tables_TableExporterTest extends StorageApiTestCase
{
	...
	public function tableImportData()
	{
		return array(
			...
			array(self::BACKEND_REDSHIFT, new CsvFile('https://s3.amazonaws.com/keboola-tests/escaping.csv'), 'escaping.backslash.out.csv', array('format' => 'escaped')),
			array(self::BACKEND_REDSHIFT, new CsvFile('https://s3.amazonaws.com/keboola-tests/escaping.csv'), 'escaping.backslash.out.csv', array('format' => 'escaped')),
			array(self::BACKEND_REDSHIFT, new CsvFile('https://s3.amazonaws.com/keboola-tests/escaping.csv'), 'escaping.raw.out.csv', array('format' => 'raw')),
			array(self::BACKEND_REDSHIFT, new CsvFile('https://s3.amazonaws.com/keboola-tests/escaping.csv'), 'escaping.raw.out.csv', array('gzip' => true, 'format' => 'raw')),
